function progress() {
  ...
  function frame() {
      if(progress == 500 && counter == 100){
          ...
          document.getElementById('loghi').style.display="none";
      } else {
          progress +=5;
          counter +=1;
    ...
    $.ajax({
      ...
      'success':function(data,state){
        for(var j = 0;j < offset; j++){
          ...
          var difference = timeStampNow - timeStamp;
          if(difference < 60){
            difference=Math.floor(difference)+ " seconds ago";
          }
          else if(difference < 3600){
            difference=Math.floor((difference/60))+ " mins ago";
          }
          else if(difference < 86400){
            var hours=Math.floor((difference/3600));
            difference= hours+ " hrs "+ Math.floor((difference%3600)/60)+ " mins ago";
          }
          else {
            var days=Math.floor((difference/86400));
            difference= days+ " days "+ Math.floor(((difference%86400)/3600))+" hrs ago"
          }
          ...
          arr.push("<a target='_blank' href=https://ropsten.etherscan.io/tx/"+txHash+">"+txHash+"</a>", blockNumber, difference, from, to, valueEther, txFeeEther);
        }
          //console.log(arr);
          macroarray[i]=arr;
    ...
    $.ajax({
      ...
      'success':function(data,state){
        for(var j = 0;j < offset; j++) {
          ...
          arr.push("<a href=https://ropsten.etherscan.io/tx/"+txHash+">"+txHash+"</a>", blockNumber, difference, from, to, value, gasUsedEther);
        }
          // console.log(arr);
          macroarray[i]=arr;
          if(arr[1]!==undefined){
